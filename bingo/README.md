...
1. Each player picks one card (or deal randomly for a group).
2. Press play on the video. The center square (⭐ FREE SPACE — Still Employed) is already marked for everyone.
3. When you hear or see a square's moment in the video, mark it. Each square counts once — first call wins ties in a group.
4. Win with 3 in a row — any row, column, or diagonal (8 possible lines on a 3×3). Center counts toward all 4 lines through it.
5. Shout "STILL EMPLOYED!" when you complete a line. For a full-watch game, keep playing for blackout (all 8 squares + free space).
6. Disputed square? Majority of the room (or the card holder's quote of the line) decides — keep the video rolling.

Tip for hosts: pause once at the start to let everyone open their card, then no pauses — the video runs just under 11 minutes.
